...
Write-Output "$(Get-Timestamp) $info Log Path: $dcsLogPath" | Out-file $debugLog -append
...
# The log index to iterate through
$logTrapIndex = 0
$lastTrapTime = $null
...
while ($true) {
    ...
    [DateTime]$lastTrapTime = $lastTrapString
...
<#  Calculate the scan interval. To make a long story short, there is a small couple hundred millisecond gap 
    ...
    #Check for last logged trap time and store it for comparison
    $lastTrapString = Select-String -Path $rawGradelog -Pattern "^.*GRADE:.*" | Select-Object -Last 1
    $lastTrapString = $lastTrapString -replace "^.*(?:.txt\:\d{0,5}\:)", ""
    $lastTrapString = $lastTrapString -replace ",.*$", ""
    [DateTime]$lastTrapTime = $lastTrapString
    ...
    try {
        ...
        Write-Output "$(Get-Timestamp) $info No new landing Event with log index" | Out-file $debugLog -append
    } 
    
    #If dcs.log did not contain any lines that matched the LSO regex, stop, otherwise continue

    if ($null -eq $landingEvent ) {
        Write-Output "$(Get-Timestamp) $info $lcDect No landing events detected" | Out-file $debugLog -append
        #Do Nothing
    }
        ...
        Write-Output "$(Get-Timestamp) $info Comparing to last trap at: $lastTrapTime" | Out-file $debugLog -append

        #Convert the log time string to a usable time object
        [DateTime]$trapTime = $logTime
        Write-Output "$(Get-Timestamp) $info DCS.log trap time: $trapTime" | Out-file $debugLog -append
        $boolCheck = $trapTime -gt $lastTrapTime
        Write-Output "$(Get-Timestamp) $info DCS.log trap is more recent than last logged trap: $boolCheck" | Out-file $debugLog -append

        if ($null -eq $lastTrapTime -or $trapTime -gt $lastTrapTime) {
            ...
            if (($Pilot -eq "System.Object[]") -or ($Grade -eq "System.Object[]")) {
            ...
            Write-Output "$(Get-Timestamp) $info After closer inspection, no new landing events" | Out-file $debugLog -append
        }
    }
    ...
    $lsoLoopDuration = New-TimeSpan -Start $lsoLoopStartSysTime -End $lsoLoopEndSysTime
    Write-Output "$(Get-Timestamp) $info $lcJob LSO BOT loop duration was $lsoLoopDuration" | Out-file $debugLog -append
    Write-Output "$(Get-Timestamp) $info $lcTime LSO BOT Cycle Ran. Sleeping for $scanInterval seconds" | Out-file $debugLog -append

    Start-Sleep -Seconds $scanInterval.TotalSeconds
}
#Garbage Collection
[system.gc]::Collect()